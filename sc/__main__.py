...
CLI_MAIN = Path(__file__).resolve()
    ...
    try:
        with open(script) as fh:
            for line in fh:
                ...
                is_match = completion_arg == str(arg)
                is_wildcard = completion_arg == "*" and allow_wildcard
                if not (is_match or is_wildcard):
                    continue
